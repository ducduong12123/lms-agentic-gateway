...
def register(reg: ToolRegistry, frappe) -> None:
    ...
    _register_mutation(reg, "manage_chapter", "Tạo, đổi tên, sắp xếp hoặc xóa chương; tự đồng bộ bảng chương của khóa học.", {
        "type": "object", "properties": {"operation": {"type": "string", "enum": ["create", "update", "reorder", "delete"]}, "course": {"type": "string"}, "chapter": {"type": "string"}, "title": {"type": "string"}, "ordered_chapters": {"type": "array", "items": {"type": "string"}}}, "required": ["operation"],
    }, manage_chapter)
    ...
    def manage_quiz(a: dict):
        ...
        if operation == "create":
            ...
            return action_result("manage_quiz", "Đã tạo quiz", f"Đã tạo quiz {fields['title']} với {len(created_questions)} câu hỏi.", changes)
        if operation != "update":
            raise ValueError("operation must be create, update or delete")
        quiz = _require(a.get("quiz"), "quiz")
        if not fields:
        ...
        return action_result("manage_quiz", "Đã cập nhật quiz", f"Đã cập nhật quiz {quiz}.", changes, undo={"op": "restore", "doctype": "LMS Quiz", "name": quiz, "fields": {key: before.get(key) for key in fields}})
    ...
    def _simple_manager(a: dict, *, doctype: str, action: str, allowed: set[str], required: tuple[str, ...]):
        ...
        if operation == "create":
            ...
            return action_result(action, f"Đã tạo {doctype}", f"Đã tạo {name}.", [_change(doctype, name, "create")])
        if operation != "update":
            raise ValueError("operation must be create, update or delete")
        _require(target, "name")
        if not fields:
    ...
    _register_mutation(reg, "manage_assignment", "Tạo, sửa hoặc xóa assignment của khóa học, gồm đề bài, loại nộp, đáp án và chế độ chấm.", {
        "type": "object", "properties": {"operation": {"type": "string", "enum": ["create", "update", "delete"]}, "name": {"type": "string"}, "title": {"type": "string"}, "question": {"type": "string"}, "type": {"type": "string", "enum": ["Document", "PDF", "URL", "Image", "Text"]}, "course": {"type": "string"}, "grade_assignment": {"type": "integer", "enum": [0, 1]}, "show_answer": {"type": "integer", "enum": [0, 1]}, "answer": {"type": "string"}}, "required": ["operation"],
    }, manage_assignment)
