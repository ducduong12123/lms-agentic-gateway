...
``lms_copilot.demo.clear`` removes it again, together with the demo learners.
...
does not exist (``copilot-demo-learner``); ``lms_copilot/demo/README.md`` describes the
...
from lms_copilot.copilot.content import terms
...
def seed():
	...
	with patch("lms_copilot.copilot.gateway.enqueue_review", return_value=False):
